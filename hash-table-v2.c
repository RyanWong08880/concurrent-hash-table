...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		int ret = pthread_mutex_init(&entry->mutex, NULL);
		if (ret != 0) {
			exit(ret);
		}
	}
                             ...
                             uint32_t value)
{
	...
	int ret = pthread_mutex_lock(&hash_table_entry->mutex);
	if (ret != 0) {
		exit(ret);
	}
	...
	ret = pthread_mutex_unlock(&hash_table_entry->mutex);
	if (ret != 0) {
		exit(ret);
	}
}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		int ret = pthread_mutex_destroy(&entry->mutex);
		if (ret != 0) {
			exit(ret);
		}
	}
