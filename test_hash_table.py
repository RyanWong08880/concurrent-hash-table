...
class TestHashTable(unittest.TestCase):
    ...
    def test_1(self):
        ...
        hash_result = subprocess.check_output(('./hash-benchmark', '-t', '8', '-s', '50000')).decode()
    ...
    def test_2(self):
        ...
        hash_result = subprocess.check_output(('./hash-benchmark', '-t', '8', '-s', '40000')).decode()
        nums = re.sub(r'Generation: ([\d\,]+) usec\nHash table base: ([\d\,]+) usec\n  - ([\d\,]+) missing\nHash table v1: ([\d\,]+) usec\n  - ([\d\,]+) missing\nHash table v2: ([\d\,]+) usec\n  - ([\d\,]+) missing\n', 
                      r'\1|\2|\3|\4|\5|\6|\7',
                      hash_result)
    ...
    def test_3(self):
        ...
        hash_result = subprocess.check_output(('./hash-benchmark', '-t', '4', '-s', '50000')).decode()
        nums = re.sub(r'Generation: ([\d\,]+) usec\nHash table base: ([\d\,]+) usec\n  - ([\d\,]+) missing\nHash table v1: ([\d\,]+) usec\n  - ([\d\,]+) missing\nHash table v2: ([\d\,]+) usec\n  - ([\d\,]+) missing\n', 
                      r'\1|\2|\3|\4|\5|\6|\7',
                      hash_result)
